...
function cleanName(value: string): string {
  return String(value || '')
    ...
    .replace(/\b\d+[,.]\d{2}\b\s*(zł|zl|pln|eur|usd|€|\$)?/gi, '')
    .replace(/\s{2,}/g, ' ')
...
function parseItemLine(line: string, fallbackCurrency: string): ParsedReceiptItem | null {
  const priceMatch = line.match(/(-?\d+[,.]\d{2})\s*(zł|zl|pln|eur|usd|€|\$)?\s*$/i);
...
export function parseReceiptText(text = '', fallbackCurrency = 'PLN'): ParsedReceipt {
  ...
  lines.forEach(line => {
    ...
    const lastPrice = line.match(/(\d+[,.]\d{2})\s*(zł|zl|pln|eur|usd|€|\$)?\s*$/i);
